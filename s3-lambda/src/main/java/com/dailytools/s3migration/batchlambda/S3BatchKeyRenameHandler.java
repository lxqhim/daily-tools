...
/** S3 Batch Operations Lambda handler that copies each task to its renamed key. */
public class S3BatchKeyRenameHandler implements RequestStreamHandler {
    ...
    public S3BatchKeyRenameHandler() {
        this.jsonCodec = new S3BatchJsonCodec();
        this.processor = new S3KeyRenameProcessor(KeyRenameConfig.fromEnvironment());
    }

    S3BatchKeyRenameHandler(S3BatchJsonCodec jsonCodec, BatchTaskProcessor processor) {
    ...
    public void handleRequest(InputStream input, OutputStream output, Context context) throws IOException {
        S3BatchLambdaResponse response = handle(jsonCodec.readEvent(input));
